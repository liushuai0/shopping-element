<template>
    <el-row class="container">
        <el-col :span="24" class="header">
            ...
            <el-col :span="10">
                <div @click.prevent="collapse" class="tools">
                    <i class="fa fa-align-justify"></i>
                </div>
            </el-col>
            <el-col :span="4" class="userinfo">
                <el-dropdown trigger="hover">
                    <span class="el-dropdown-link userinfo-inner" @click="toWelcome()"><img
        ...
        <el-col :span="24" class="main">
                ...
                <ul class="el-menu el-menu-vertical-demo collapsed" ref="menuCollapsed" v-show="collapsed">
                    <li class="el-submenu item" v-for="(item,index) in $router.options.routes" v-if="!item.hidden">
                        <template v-if="!item.leaf">
                            <div @mouseout="showMenu(index,false)" @mouseover="showMenu(index,true)"
                                 class="el-submenu__title"
                                 style="padding-left: 20px;"><i :class="item.iconCls"></i></div>
                            <ul :class="'submenu-hook-'+index" @mouseout="showMenu(index,false)"
                                @mouseover="showMenu(index,true)"
                                class="el-menu submenu">
                                <li :class="$route.path==child.path?'is-active':''" :key="child.path"
                                    @click="$router.push(child.path)"
                                    class="el-menu-item" style="padding-left: 40px;"
                                    v-for="child in item.children" v-if="!child.hidden">
                                    {{child.name}}
                                </li>
                            </ul>
                        </template>
                            ...
                            <ul>
                                <li class="el-submenu">
                                    <div :class="$route.path==item.children[0].path?'is-active':''"
                                         ...
                                         style="padding-left: 20px;height: 56px;line-height: 56px;padding: 0 20px;"><i
            ...
            <section class="content-container" style="background-color: #eaedf1">
...
<script>
    ...
    export default {
        ...
        methods: {
            async getMenuList() {
                let _result = await getMenuList()
                if (_result.meta.status !== 200) {
                    return this.$message.error("获取菜单失败")
                }
                console.table(_result.data)
                this.menuList = _result.data
            },
            onSubmit() {
                console.log('submit!');
            },
